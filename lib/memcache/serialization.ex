defmodule Memcache.Serialization do
  ...
  def encode_request(header, key, body \\ "", extras \\ "") do
    ...
    <<0x80::size(8), opcode::size(8), key_length::size(16), extras_length::size(8), 0::size(8),
      0::size(16), total_body_length::size(32), 0::size(32), cas::size(64), total_body::binary>>
  end

  def decode_response_header(
        <<0x81::size(8), opcode::size(8), key_length::size(16), extras_length::size(8),
          _data_type::size(8), status::size(16), total_body_length::size(32), _opaque::size(32),
          cas::size(64)>>
      ) do
    {:ok,
     %Header{
...
defmodule Memcache.Serialization.Opcode do
  def quiet?(opcode) do
    opcode in [
      :getq,
      :getkq,
